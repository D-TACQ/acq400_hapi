...
def run_test(args):
    ...
    while counter < args.loops:
        ...
        for uut in args.uuts:
            uut.s0.set_arm = 1
            acq400_hapi.shotcontrol.wait_for_state(
                uut, "ARM")  # Pass hostname and required state
        print "Set arm"
        ...
        for uut in args.uuts:
            acq400_hapi.shotcontrol.wait_for_state(
                uut, "IDLE")  # Pass hostname and required state
        ...
        for uut in args.uuts:
            # number of post shot samples
            samples.append(uut.s0.TRANS_ACT_POST)
...
def run_main():
    ...
    parser.add_argument('--loops', default=sys.maxint,
                        type=int, help="Number of loops to run")
    parser.add_argument('uuts', nargs='+',
                        help='UUTs - master should be first')
