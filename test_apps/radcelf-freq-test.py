...
def verify_freq(uut, test, freqA, freqB):
    ...
    while retry < 10:
        if valid_freq(freq(uut.s2.SIG_DDS_A_FREQ), freqA) and valid_freq(freq(uut.s2.SIG_DDS_B_FREQ), freqB):
            print("test:%d  PASS %s %s" % (test, freq(
                uut.s2.SIG_DDS_A_FREQ), freq(uut.s2.SIG_DDS_B_FREQ)))
            return True
        else:
            time.sleep(1)
            retry = retry + 1

    print("test:%d FAIL %s != %s %s != %s" % (test, freqA, freq(
        uut.s2.SIG_DDS_A_FREQ), freqB, freq(uut.s2.SIG_DDS_B_FREQ)))
    return False
