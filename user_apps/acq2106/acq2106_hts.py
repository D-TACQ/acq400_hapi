...
example usage::
    ./user_apps/acq2106/acq2106_hts.py acq2106_133 acq2106_176

    ./user_apps/acq2106/acq2106_hts.py --nbuffers=2000 acq2106_133 acq2106_176

    ./user_apps/acq2106/acq2106_hts.py ---map=133:A:1/133:B:2 --secs=30 acq2106_133

    ./user_apps/acq2106/acq2106_hts.py --map=643:A:ALL/124:BOTH:3,4 --nbuffers=1000 192.168.1.1 192.168.1.2



usage::
    acq2106_hts.py [-h] [--clk CLK] [--trg TRG] [--sim SIM]
                     [--secs SECS] [--spad SPAD] [--decimate DECIMATE] [--map MAP]
...
def get_parser():    
    ...
    parser.add_argument('--nbuffers', type=int, default=1000, help='set capture length in buffers')
    parser.add_argument('--secs', default=0, type=int, help="capture seconds [default:0 until buffer full]")
    ...
    parser.add_argument('--delete', default=1, help='delete stale data')
...
def configure_shot(uuts, args):
    for uut in uuts.items():
        ...
        if args.sig_gen:
            ...
            hapi.s1.TRG_DX = 'd1'
        #PR.Purple(hapi.s1.help())
        hapi.s1.ACQ480_FIR_01 = 1 #remove me
...
def initialize_streams(uuts,args):
    ...
    for uut in uuts:
        for stream in uuts[uut]['SRM']:
            #PR.Blue(f'Starting stream {stream}')
            cmd = 'sudo ./scripts/run-stream-ramdisk {} {}'
            cmd = cmd.format(stream, nbuffers).split()
    ...
    try:
        while True:
            ...
            if args.secs and all_running:
                ...
                SCRN.add("{0:.1f}/{1} secs or ",count,args.secs)
            else:
                SCRN.add("{0:.1f} secs ",time.time() - time_start)
